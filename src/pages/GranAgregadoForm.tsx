...
export default function GranAgregadoForm() {
    ...
    const [draftData, setDraftData] = useState<GranAgregadoPayload | null>(null)
    ...
    const save = useCallback(
        async (download: boolean) => {
            ...
            try {
                ...
                if (download) {
                    const { blob, filename } = await saveAndDownloadGranAgregadoExcel(payload, editingEnsayoId ?? undefined)
                    const url = URL.createObjectURL(blob)
                    const a = document.createElement('a')
                    a.href = url
                    a.download = filename || `${buildFormatPreview(form.muestra, 'AG', 'GR. AGREGADO')}.xlsx`
                    a.click()
    ...
    return (
        <div className="min-h-screen bg-slate-100 p-4 md:p-6">
            ...
            <FormatConfirmModal
                ...
                formatLabel={buildFormatPreview(form.muestra, 'AG', 'GR. AGREGADO')}
